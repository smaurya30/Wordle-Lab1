...
class UI():
    ...
    def __init__(self):
        ...
        # except:
        #     # DB Entry
        #     self.dataB.insertError("Error:", sys.exc_info()[0], " in Gameplay Log, UI module, occurred.".__str__())
    ...
    def checkWordle(self, setOfEnteredWords, word, wordleWordList, gamesPlayed, winCount, count, flag, Counter, badWordSet, greenLetters, yellowLetters):
        try:
            print("nextWord : ", word)
            wordleWordListCopy = ''.join(wordleWordList)
            wordList = list(word)
            ...
            # increment the user attempt after updating the colors
                ...
                result, badWordSet, greenLetters, yellowLetters, setOfEnteredWords, nextWord = wm.wordleFunc(badWordSet, greenLetters, yellowLetters, setOfEnteredWords)
                # wm.wordleFunc()
                # result = wm.resultString
                
                # DB Entry
                resultCopy = ''.join(result)
                id = str(uuid.uuid4())
                self.dataB.insertData(id, wordleWordListCopy, word, resultCopy)
                self.dataB.insertDataWinCount(id, str(gamesPlayed), str(winCount))
            ...
            return setOfEnteredWords, gamesPlayed, winCount, count, flag, Counter, badWordSet, greenLetters, yellowLetters, nextWord
        except:
            # DB Entry
            self.dataB.insertError("Error: {error} in checkWordle method, UI module, occurred.".format(error = sys.exc_info()[0]))
    ...
    def wordleUi(self, wordleWord, flag, winCount, Counter, gamesPlayed):
        try:
            ...
            yellowLetters = dict()
            # yellowLetters = set()
            print("1st guess word: ",wordRankFile[0].split(',')[1])
            word = wordRankFile[0].split(',')[1]
            while count < 7:
                wordleWordList = list(wordleWord)
                print("Entered 5 letter word".__str__())
                # word = input().lower()
                # logging.info("Input word: "+word)
                setOfEnteredWords, gamesPlayed, winCount, count, flag, Counter, badWordSet, greenLetters, yellowLetters, nextWord = self.checkWordle(setOfEnteredWords, word, wordleWordList, gamesPlayed, winCount, count, flag, Counter, badWordSet, greenLetters, yellowLetters)
                word = nextWord
                if flag == -1:
            ...
            # DB Entry
            self.dataB.insertError("Error: {error} in wordleUi method, UI module, occurred.".format(error = sys.exc_info()[0]))
    ...
    def statsFuncs(self):
        try:
            ...
            # DB Entry
            self.dataB.insertError("Error:", sys.exc_info()[0], " in statsFuncs method, UI module, occurred.".__str__())
    ...
    def main(self):
        try:
            ...
            self.dataB.getData('2022-04-27','2022-04-28')
            
        except:
            # DB Entry
            self.dataB.insertError("Error: {error} in main method, UI module, occurred.".format(error = sys.exc_info()[0]))
