...
try:
    ...
    print("Error:", sys.exc_info()[0], " in Gameplay Log, UI module, occurred.")
'''
Psuedocode:
...
def checkWordle(setOfEnteredWords, word, wordleWordList, gamesPlayed, winCount, count, flag, Counter):
    try:
        ...
        print("Error:", sys.exc_info()[0], " in checkWordle method, UI Module, occurred.")
...
def wordleUi(wordleWord, flag, winCount, Counter, gamesPlayed):
    try:
        ...
        print("Error:", sys.exc_info()[0], " in wordleUi method, UI Module occurred.")

        
def statsFuncs():
    try:
        letterFreqDictRes = statistics.letterFreq()
        convertLetterFreqDictRes = statistics.convertListToTuple(letterFreqDictRes.copy())
        ...
        freqDictTupleParseRes = statistics.writeTupleDictInFile()
        print('Parse the statistics file into a dictionary of tuples - 3: ',freqDictTupleParseRes)
        wordRankRes = statistics.wordRank(letterFreqDictRes)
    except:
        print(sys.exc_info())
...
def main():
    try:
        ...
        print("Error:", sys.exc_info()[0], " in main, UI Module, occurred.")
